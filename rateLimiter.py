...
def _addIPAddress(IPAddress, cur, userID = None):
    if userID:
        ...
        postgreSQL_insert_Query = f""" INSERT INTO {IP_table} ({IP_addresses, IP_requests, IP_start_requests, IP_users}) VALUES ({IPAddress}, {0}, {datetime.now()}, {[]})"""
...
def _addUser(userID, IPAddress, IP_info, cur):
    ...
    postgreSQL_update_query = f"""Update {IP_table} set {IP_users} = {new_IP_info} where {IP_addresses} = {IPAddress}"""
